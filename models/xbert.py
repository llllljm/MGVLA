...
def load_tf_weights_in_bert(model, config, tf_checkpoint_path):
    """Load tf checkpoints in a pytorc，h model."""
    try:
        import re
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # ttention_mask，[seq_len, seq_len], 代表两不同位置间是否做attention
...
class BertAttention(nn.Module):
    ...
    def prune_heads(self, heads):
        # 利用attention mask，需要prune的head，其mask为1。保留的head则mask为0
        if len(heads) == 0:
            return
...
class BertPooler(nn.Module):
    def __init__(self, config):
        # pooler层对CLS位置向量，进行全连接和tanh激活，从而得到输出向量
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        # 从pretrain model加载初始化weights，多头剪枝等
        self.init_weights()
