...
class Evaluator:
    ...
    def evaluate(self, eval_dataloader=None):
        ...
        if self.mode == 'binary':
            if pred.shape[1] == 1:
                pred_score = torch.tensor(pred).numpy().flatten()
                ...
                for index, score in enumerate(pred):
                    ...
                    pred_score = torch.tensor(score[inds[0]]).numpy()
                    pred_scores.append(pred_score)
                pred_scores = np.array(pred_scores).flatten()
                max = np.max(pred_scores)
                min = np.min(pred_scores)
                pred_scores = (pred_scores - min) / (max - min)
                pred_label = np.ones(len(pred))
                pred_label[pred_scores < 0.5] = 0
                acc = (pred_label == labels).mean()
                outputs['acc'] = acc
        ...
        if self.mode == 'multiclass':
            ...
            for index, score in enumerate(pred):
                inds = np.argsort(score)[::-1]
                pred_label = sentence_label[inds[0]]
                if pred_label == labels[index]:
                    num += 1
            acc = num / 1000
            outputs['acc'] = acc
