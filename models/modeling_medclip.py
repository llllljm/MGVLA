...
class MedCLIPVisionModelViT(nn.Module):
    ...
    def __init__(self, checkpoint=None, medclip_checkpoint=None) -> None:
        ...
        '''
        super().__init__()
        self.vit_type = 'microsoft/swin-tiny-patch4-window7-224'
    ...
    def forward(self, pixel_values, project=True):
        ...
        img_feat = img_feats.unsqueeze(1)
        img_embeds = torch.cat((img_feat, img_embeds),dim = 1)
        # if project:
        #     img_feats = self.projection_head(img_feats)
        return img_embeds, img_feats
...
class MedCLIPModel(nn.Module):
    ...
    def from_pretrained(self, input_dir=None):
        ...
        if isinstance(self.vision_model, MedCLIPVisionModel):
            # resnet
            pretrained_url = 'https://github.com/RyanWangZf/MedCLIP/raw/main/medclip/medclip_resnet_weight.txt'
            if input_dir is None:
                input_dir = './pretrained/medclip-resnet'
        elif isinstance(self.vision_model, MedCLIPVisionModelViT):
            # ViT
            pretrained_url = 'https://github.com/RyanWangZf/MedCLIP/raw/main/medclip/medclip_vit_weight.txt'
            if input_dir is None:
        ...
        state_dict = torch.load(os.path.join(input_dir, 'pytorch_model.bin'))
    ...
    def forward(self,
        ...
        **kwargs,
        ...
        if return_loss:
            ...
            # loss = self.soft_clip_loss(logits_per_image, label_sim)
        else:
            loss = None
...
class PromptClassifier(nn.Module):
    ...
    def forward(self, pixel_values=None, prompt_inputs=None, **kwargs):
        ...
        for cls_name, cls_text in prompt_inputs.items():
            ...
            image_embed, image_feat = self.model.visual_encoder(inputs.pixel_values)
            img_feat = F.normalize(self.model.vision_proj(image_feat), dim=-1)
            output = self.model.text_encoder(inputs.input_ids, attention_mask=inputs.attention_mask)
            text_embed = output.last_hidden_state
            text_feat = F.normalize(self.model.text_proj(text_embed[:, 0, :]))
            logits = self.model.compute_logits(img_feat, text_feat)
            # take logits max as the class similarity8;
